...
function EventForm({ method, event }) {
  ...
  return (
    // if you have action on another route then set by action="path"
...
export async function action({ request, params }) {
  ...
  // console.log("eventData From newEvent Action: ", eventData);

  let url = "http://localhost:8989/events";

  if (method === "PATCH") {
    const eventId = params.eventId;
    url = "http://localhost:8989/events/" + eventId;
  }

    // get and sent token for auth.
